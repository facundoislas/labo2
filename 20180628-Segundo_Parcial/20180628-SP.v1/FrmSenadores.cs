...
namespace _20180628_SP.v1
{
    public partial class FrmSenadores : Form
    {
        ...
        public void ManejadorVoto(string senador, Votacion.EVoto voto)
        {
            ...
            else
            {
                ...
                if (aux == 0)
                {
                    ...
                    votacion.ContadorAfirmativo = short.Parse(lblAfirmativo.Text);
                    votacion.ContadorNegativo = short.Parse(lblNegativo.Text);
                    votacion.ContadorAbstencion = short.Parse(lblAbstenciones.Text);
                    SerializarBinaria bin = new SerializarBinaria();
                    bin.Guardar(votacion.NombreLey, votacion);
                    Dao dao = new Dao();
                    dao.Guardar(votacion.NombreLey, votacion);
        ...
        {
            SerializarBinaria bin = new SerializarBinaria();
            //votacion = bin.Leer("prueba");
            if (!ReferenceEquals(votacion, null))
            {
                lblAfirmativo.Text = votacion.ContadorAfirmativo.ToString();
                lblNegativo.Text = votacion.ContadorNegativo.ToString();
                lblAbstenciones.Text = votacion.ContadorAbstencion.ToString();
            }
            else
                MessageBox.Show("No hubo Votacion","Alerta", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        }
